...
$(function () {$('#load10more').bind("click", function () {
    //FIXME some matches on the same day might be wrong order
    loadTen(matchTracking['matches'].splice(matchTrackingIndex,matchTrackingIndex+10))
    matchTrackingIndex += 10
  });
  });
...
async function loadTen(matches){
    matches.forEach(async matchId => {
        var matchData = await loadOne(matchId);
...
function createMatchDiv(match){
    ...
    let meta = $('<div></div>').addClass('flex flex-col w-36 place-items-center justify-center');
    let teamsDiv = $('<div></div>').addClass('flex gap-4')
    let objectives = $('<div></div>').addClass('flex-row flex order-2 gap-4 hidden items-center objectivDiv')
    ...
    let date = $('<span>'+dateValue.getHours()+':'+dateValue.getMinutes()+ ' '+dateValue.getDate() +'/'+ (dateValue.getMonth()+1)+ '</span>')
    let year = $('<span class="hidden">-'+String(dateValue.getFullYear()).slice(-2)+ '</span>')
    let length = $('<p>'+Math.floor(match['info']['gameDuration']/6)/10 + ' m</p>')
    let queueId = $(`<p>${queueIdConverter[match['info']['queueId']]}:${match['metadata']['averageRank']['tier']} ${match['metadata']['averageRank']['division']}</p>`)
    meta.prepend([length, date,year,queueId])

    let teamsPlayers = {'100' : '', '200' : ''}
    var totalKills = {'100' : match['info']['teams'][0]['objectives']['champion']['kills'],
    ...
    match['info']['participants'].forEach(player => {
        ...
        goldMatchups[player['teamPosition']][player['teamId']] = player['goldEarned']
        if(player['puuid'] != summoner['puuid']){
            return
        }
        userPos = player['teamPosition']
        teams[player['teamId']].addClass('order-1 bg-gray-400 flex flex-col')
        teams[Math.abs(player['teamId']-300)].addClass('order-3 bg-gray-400 flex flex-col')
    ...
    let teamsObjectives100 = $(`<div>
        <p class='h-8'>${match['info']['teams'][0]['objectives']['champion']['kills']}</p>
        <p class='h-8'>${match['info']['teams'][0]['objectives']['dragon']['kills']}</p>
        <p class='h-8'>${match['info']['teams'][0]['objectives']['baron']['kills']}</p>
        <p class='h-8'>${match['info']['teams'][0]['objectives']['riftHerald']['kills']}</p>
        <p class='h-8'>${match['info']['teams'][0]['objectives']['tower']['kills']}</p>
        <p class='h-8'>${match['info']['teams'][0]['objectives']['inhibitor']['kills']}</p>
    ...
    let teamsObjectives200 = $(`<div>
        <p class='h-8'>${match['info']['teams'][1]['objectives']['champion']['kills']}</p>
        <p class='h-8'>${match['info']['teams'][1]['objectives']['dragon']['kills']}</p>
        <p class='h-8'>${match['info']['teams'][1]['objectives']['baron']['kills']}</p>
        <p class='h-8'>${match['info']['teams'][1]['objectives']['riftHerald']['kills']}</p>
        <p class='h-8'>${match['info']['teams'][1]['objectives']['tower']['kills']}</p>
        <p class='h-8'>${match['info']['teams'][1]['objectives']['inhibitor']['kills']}</p>
    ...
    let teamsObjectivesMiddle = $(`<div>
        <img src="/static/pics/objectives/champion.png" class='h-8 w-8'>
        ...
        <img src="/static/pics/objectives/coin.png" class='h-8 w-8'>
    
        </div>`).addClass('flex flex-col xl:flex teamsObjectivesMiddle')
    objectives.prepend([teamsObjectives100, teamsObjectivesMiddle,teamsObjectives200])

    match['info']['participants'].forEach(player => {
        //console.log(player)
        let username = $(`<div><p>${player["summonerName"]}</p></div>`).addClass('w-fit text-clip ')
        let rank = $(`<P>${player["rank"]} | ${player["summonerLevel"]} lvl</p>`)
        let summonerDiv = $(`<div></div>`).prepend([username, rank]).addClass('w-36 truncate items-center place-items-center flex flex-col flex-nonwrap ')

        let lane = $(`<div><img src='/static/pics/roles/${player["teamPosition"]}.png' class='w-10 pr-2'></div>`).addClass('relative flex place-items-center lanesIMG')

        let champ = $(`<img src='http://ddragon.leagueoflegends.com/cdn/${$GAME_VERSION}/img/champion/${player['championName']}.png' ></img>`).addClass('w-12 ')
        let champlvl = $(`<div><div class='bg-gray-300 opacity-90 leading-4'>${player['champLevel']}</div></div>`).addClass('absolute  top-11 z-1')
        let champDiv = $(`<div></div>`).prepend([champ,champlvl]).addClass('relative flex place-items-center')

        let sum1 = $(`<img src="http://ddragon.leagueoflegends.com/cdn/8.11.1/img/spell/${summonerIdToName[player['summoner1Id']]}" class='w-6'></img>`)
        let sum2 = $(`<img src="http://ddragon.leagueoflegends.com/cdn/8.11.1/img/spell/${summonerIdToName[player['summoner2Id']]}" class='w-6 '></img>`)
        let sums = $(`<div></div>`).prepend([sum1, sum2]).addClass('relative flex-col flex place-items-center self-center ')

        let runePath = 'https://raw.communitydragon.org/latest/plugins/rcp-be-lol-game-data/global/default/v1/'
        let keystone = $(`<img src="${runePath}${runeIdToPath[player['perks']['styles'][0]['selections'][0]['perk']]}" class='w-6 max-w-full'></img>`)
        let secondTreeLogo = $(`<img src="${runePath}${runeStyleIdToPath[player['perks']['styles'][1]['style']]}" class='w-6 max-w-full'></img>`)
        let easyRunes = $('<div></div>').prepend([keystone, secondTreeLogo]).addClass('bg-gray-700')
        let primaryTree = $(`<div>
        <img src='${runePath}${runeIdToPath[player['perks']['styles'][0]['selections'][1]['perk']]}' class='w-6 max-w-full'></img>
        <img src='${runePath}${runeIdToPath[player['perks']['styles'][0]['selections'][2]['perk']]}' class='w-6 max-w-full'></img>
        <img src='${runePath}${runeIdToPath[player['perks']['styles'][0]['selections'][3]['perk']]}') class='w-6 max-w-full'></img>
        </div>`).addClass('flex flex-row max-w-full')

        let secondTree = $(`<div>
        <img src='${runePath}${runeIdToPath[player['perks']['styles'][1]['selections'][0]['perk']]}' class='w-6 max-w-full'></img>
        <img src='${runePath}${runeIdToPath[player['perks']['styles'][1]['selections'][1]['perk']]}' class='w-6 max-w-full'></img>
        </div>`).addClass('flex flex-row max-w-full')

        let statTree = $(`<div>
        <img src='${runePath}${runeIdToPath[player['perks']['statPerks']['offense']]}' class='w-6 '></img>
        <img src='${runePath}${runeIdToPath[player['perks']['statPerks']['flex']]}' class='w-6 '></img>
        <img src='${runePath}${runeIdToPath[player['perks']['statPerks']['defense']]}' class='w-6 '></img>
        </div>`).addClass('flex flex-row max-w-full')
        let fullTree = $(`<div></div>`) .prepend([primaryTree,secondTree,statTree ]).addClass(' max-w-full flex-col absolute bg-black group-hover:opacity-100 opacity-0 z-20 ')
        let runes = $(`<div></div>`).prepend([easyRunes, fullTree]).addClass('items-center place-items-center flex flex-row flex-nonwrap group ')

        
        let itemsValue = [player['item0'], player['item1'], player['item2'], player['item3'], player['item4'], player['item5']]
        ...
        let items = $('<div></div>').prepend([itemsTop, itemsBot]).addClass('flex flex-col ')
        let itemsAll = $('<div></div>').prepend([items, itemsWard]).addClass('flex flex-row items-center place-items-center flex-nonwrap ')



        let killsDeathAssists = $(`<p>${player['kills']}/${player['deaths']}/${player['assists']}</p>`)
        let kda
        if(player['deaths'] == 0){
            kda = $(`<p >Nice</p>`);
        }else{
            kda = $(`<p>${Math.floor(10*(player['kills']+player['assists'])/player['deaths'])/10}(${Math.floor(100*(player['kills']+player['assists'])/totalKills[player['teamId']])}%)</p>`)
        }
        let killsHelp = $(`<div style="color:white"><p>kills/deaths/assists</p><p>kda </p> <p> kill participation</p></div>`).addClass('group-hover:opacity-100 opacity-0  z-10 bg-gray-500 text-my-color decoration-white text-center absolute')
        let kills = $(`<div></div>`).prepend([killsDeathAssists, kda]).addClass('w-20 items-center place-items-center flex flex-col flex-nonwrap group')


        let visionScore = $(`<p>${player["visionScore"]}(${player["visionWardsBoughtInGame"]}/${player["wardsKilled"]}/${player["wardsPlaced"]})</p>`).addClass('whitespace-nowrap')
        let cs = $(`<p>${player["totalMinionsKilled"]}(${Math.floor(10*player["totalMinionsKilled"]/(Math.floor(match['info']['gameDuration']/6)/10))/10})</p>`)
        let goldDiff = $(`<p>${Math.floor((player["goldEarned"]-goldMatchups[player['teamPosition']][Math.abs(player['teamId']-300)])/100)/10}k</p>`)
        let randomHelp = $(`<div style="color:white">
        <p>vision score(control wards/wards killed/wards placed)</p>
        <p>cs(cs/min)</p> 
        <p>matchup gold difference</p>
        </div>`).addClass('group-hover:opacity-100 opacity-0 bg-gray-500 text-my-color decoration-white text-center absolute')
        let random = $(`<div></div>`).prepend([visionScore, cs, goldDiff]).addClass('w-28 items-center place-items-center  flex-col group  2xl:flex random')
        
        //FIXME objective damage isnt working
        let damage = $(`<p>${Math.floor(player['totalDamageDealtToChampions']/1000)}k/${ Math.floor(player['damageDealtToBuildings']/100)/10}k/${Math.floor(player['damageDealtToObjectives'] - player['damageDealtToObjectives'])}</p>`)// champions/buildings/objectives
        let heal = $(`<p>${ Math.floor((player['totalHeal']-player['totalHealsOnTeammates'])/100)/10}k/${player['totalHealsOnTeammates']}/${player['totalHealsOnTeammates']}</p>`) //     selfheal/teamheal/shield
        let cc = $(`<p>${player['totalTimeCCDealt']}s</p>`)//     cc
        let damageDiv = $(`<div></div>`).prepend([damage, heal, cc]).addClass('w-44 items-center place-items-center flex-col group  xl:flex damageDiv')


        let playerDiv = $('<div></div>').prepend([summonerDiv, lane, champDiv, sums, runes, itemsAll, kills,damageDiv, random]).addClass(' flex playerDiv hidden '+player['puuid'])
        if(player['puuid'] == summoner['puuid']){
...
summonerIdToName = {'1': 'SummonerBoost.png',
...
runeIdToPath = {'8369': '/perk-images/styles/inspiration/firststrike/firststrike.png', 
           ...
           '8014': '/perk-images/styles/precision/coupdegrace/coupdegrace.png',
            ...
            '8205': '/perk-images/styles/runesicon.png',
             ...
             '8358': '/perk-images/styles/inspiration/masterkey/masterkey.png',
              '8208': '/perk-images/styles/runesicon.png', '8347': '/perk-images/styles/inspiration/cosmicinsight/cosmicinsight.png',
...
runeStyleIdToPath = {'8000': 'perk-images/styles/7201_precision.png',
