...
$(function () {
  $('#winrates').bind('click', WR);
});

$(function () {
  $('#timeOfDay').bind('click', WR);
});

function noData() {
  $('#DivwinRates').text('there was no data');
  $('#DivtimeOfDay').text('there was no data');
  console.log();
}
  ...
  for (let index = 0; index < 24; index++) {
    ...
    $('#timeOfDayTable').append(`
        <tr id='hour${index}' class='shadow-lg bg-white'>
            <th class="bg-blue-100 border text-left px-2 py-1">${index}</th>
            <th class="bg-blue-100 border text-left px-2 py-1">${wins}/${losses}</th>
            <th class="bg-blue-100 border text-left px-2 py-1">${
              Math.round((wins * 10000) / (losses + wins)) / 100
            }</th>
        </tr>
        `);
    res.append;
